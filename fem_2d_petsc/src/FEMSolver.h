...
#include <omp.h>
#include <vector>
#include <petscksp.h> // PETSc header for linear solvers

#include <Eigen/Core>
#include <Eigen/Dense>


// Ensure PETSc is initialized with C++ compatibility
#undef __FUNCT__
#define __FUNCT__ "FEMSolver"
...
public:
    FEMSolver(int Nx, int Ny, double Lx, double Ly, double f, int rank, int size)
        : Nx(Nx), Ny(Ny), Lx(Lx), Ly(Ly), f(f), rank(rank), mesh(Nx, Ny, Lx, Ly, rank, size), 
    ...
    void run() {
        assemble();
        PetscErrorCode ierr = solve();
        if (ierr != 0) {
            std::cerr << "Rank " << rank << ": Solve failed with error code " << ierr << std::endl;
            MPI_Abort(MPI_COMM_WORLD, ierr);
        }
    }

    void printSolution();  // New function declaration
};
